...
此题经检查，存在缺失值，但是被预先命名为`null`了, 以及因此需要将这些为`null`的值清除

```{r}
missing_count = sum(is.na(tv_data$剧名) | tv_data$剧名 == "" | tv_data$剧名 == "null") # 检查剧名中有几个缺失值（NA或为空的，包括“null”字符串）
cat("剧名缺失或为空的行数是:", missing_count, "\n")


tv_data_clean = tv_data[!is.na(tv_data$剧名) & tv_data$剧名 != "" & tv_data$剧名 != "null", ] # 清除剧名为NA、为空字符串或“null”的行

summary(tv_data_clean)
```
```{r}
 mean(tv_data_clean$得分, na.rm = TRUE)
```

由此可见数据中还有其他情况，欲计算均值还得进一步处理。


**c**. 不考虑缺失数据影响，计算电视剧的平均得分。

```{r}
mean_score = mean(tv_data_clean$得分, na.rm = TRUE) #忽略缺失值的情况下计算样本均值
...
```{r}
tv_scoring = as.numeric(tv_data_clean$得分) #将非数值变量转换为NA
...
由结果可知，`飞行游戏`这一类游戏虽然波动最小（标准差最低），但是平均评分不高，显然作为玩家会更青睐有优秀作品的类型诸如拥有高的最高评分的`益智休闲`以及`策略塔防`的游戏类别。`游戏工具`这一类游戏，平均分，最高分，最低分都是最低的，与此同时波动还大；此外其他类型的游戏之间差距不是太大
